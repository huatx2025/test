...
const selectAccount = (account) => {
  ...
  if (account.platform_type === 'wechat') {
    // 解析 auth_data 获取 token 和 cookies
    let authData = {}
    try {
      authData = typeof account.auth_data === 'string' 
        ? JSON.parse(account.auth_data) 
        : account.auth_data
    } catch (e) {
      // Ignore parse error
    }
    
    const token = authData.token || account.platform_id || '0'
    const cookies = authData.cookies || ''
    
    // 通知主进程加载微信页面
    if (window.ipcRenderer) {
      window.ipcRenderer.send('toMain', {
        tag: 'wechat:loadAccount',
        data: {
